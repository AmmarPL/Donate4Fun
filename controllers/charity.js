...
router.post('/signup', (req, res, next) => {
  ...
  if (!validator.contains(req.body.name)) validationErrors.push({ msg: 'Please enter the charity name.' });
  if (!validator.contains(req.body.street)) validationErrors.push({ msg: 'Please enter your street address.' });
  if (!validator.contains(req.body.city)) validationErrors.push({ msg: 'Please enter your city.' });
  if (!validator.contains(req.body.state)) validationErrors.push({ msg: 'Please enter your state.' });
  if (!validator.contains(req.body.zip)) validationErrors.push({ msg: 'Please enter your zip code.' });
  if (!validator.isMobilePhone(req.body.phone)) validationErrors.push({ msg: 'Please enter a valid phone number.' });

  if (req.body.password !== req.body.confirmPassword) validationErrors.push({ msg: 'Passwords do not match' });

  if (validationErrors.length) {
    req.flash('errors', validationErrors);
    return res.redirect('/charity/signup');
  }
  req.body.email = validator.normalizeEmail(req.body.email, { gmail_remove_dots: false });

  const user = new Charity({
  ...
  Charity.findOne({ email: req.body.email }, (err, existingUser) => {
    if (err) { return next(err); }
    if (existingUser) {
...
router.post('/account/profile', (req, res, next) => {
  ...
  if (validationErrors.length) {
    req.flash('errors', validationErrors);
    return res.redirect('/charity/account');
  }
  req.body.email = validator.normalizeEmail(req.body.email, { gmail_remove_dots: false });

  Charity.findById(req.user.id, (err, user) => {
    if (err) { return next(err); }
    if (user.email !== req.body.email) user.emailVerified = false;
    user.email = req.body.email || '';
    user.profile.name = req.body.name || '';
    user.profile.phone = req.body.phone || '';
    user.profile.address.street = req.body.street || '';
    user.profile.address.city = req.body.city || '';
    user.profile.address.state = req.body.state || '';
    user.profile.address.zip = req.body.zip || '';
    for(var i=0; i<req.body.branches; i++) {
      user.profile.branches.append({
        street = req.body.branch[i].street || '',
        city = req.body.branch[i].city || '',
        state = req.body.branch[i].state || '',
        zip = req.body.branch[i].zip || ''
      })
    }
    user.save((err) => {
      if (err) {
...
router.post('/account/delete', (req, res, next) => {
  Charity.deleteOne({ _id: req.user.id }, (err) => {
    if (err) { return next(err); }
    req.logout();
 ...
 */
router.get('/account/verify/:token', (req, res, next) => {
...
router.post('/forgot', (req, res, next) => {
  ...
  if (validationErrors.length) {
    req.flash('errors', validationErrors);
    return res.redirect('/charity/forgot');
  }
  req.body.email = validator.normalizeEmail(req.body.email, { gmail_remove_dots: false });
